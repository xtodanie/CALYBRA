...
export default function MatchesPage() {
  ...
  const [proposedMatches, setProposedMatches] = useState(initialProposedMatches);
  const [confirmedMatches, setConfirmedMatches] = useState(initialConfirmedMatches);

  const handleConfirm = (matchToConfirm: typeof initialProposedMatches[0]) => {
  ...
  const handleReject = (matchToReject: typeof initialProposedMatches[0]) => {
    setProposedMatches(current => current.filter(m => m.id !== matchToReject.id));
    // In a real app, this would likely create an exception
  }

  const MatchTable = ({ matches, isProposed }: { matches: any[], isProposed: boolean }) => (
     <Card>
