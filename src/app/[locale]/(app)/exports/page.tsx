...
import { MonthCloseStatus } from '@/lib/types';
...
export default function ExportsPage() {
  ...
  const monthStatus = useMemo<MonthCloseStatus>(
    () => MonthCloseStatus.IN_REVIEW,
    []
  ); // MOCK: MonthCloseStatus.FINALIZED to enable

  const isFinalized = monthStatus === MonthCloseStatus.FINALIZED;
      ...
      <Card>
        <CardHeader>
          <div className="flex items-start gap-4">
            <Button onClick={handleGenerate} disabled={isGenerating || !isFinalized}>
              {isGenerating ? (
