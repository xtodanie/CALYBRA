...
*   **`[locale]/month-closes/page.tsx`**
    *   **Query:** `collection('tenants', user.tenantId, 'monthCloses')`, ordered by `periodStart` descending.
...
*   **`[locale]/month-closes/[id]/page.tsx` (Detail Page)**
    *   **Query:** `doc('tenants', user.tenantId, 'monthCloses', params.id)`.
...
*   **`[locale]/upload/page.tsx`**
    *   **Query 1 (Files):** `collection('tenants', user.tenantId, 'fileAssets')` where `monthCloseId == user.activeMonthCloseId`.
    *   **Query 2 (Jobs):** `collection('jobs')` where `monthCloseId == user.activeMonthCloseId` and `tenantId == user.tenantId`.
...
*   **`[locale]/matches/page.tsx`**
    *   **Query (Proposed):** `collection('tenants', user.tenantId, 'matches')` where `monthCloseId == user.activeMonthCloseId` and `status == 'PROPOSED'`.
    *   **Query (Confirmed):** `collection('tenants', user.tenantId, 'matches')` where `monthCloseId == user.activeMonthCloseId` and `status == 'CONFIRMED'`.
...
*   **`[locale]/exceptions/page.tsx`**
    *   **Query:** `collection('exceptions')` where `tenantId == user.tenantId` and `monthCloseId == user.activeMonthCloseId` and `status == 'OPEN'`.
